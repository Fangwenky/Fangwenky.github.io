...
export function createDeployService(options = {}) {
    ...
    async function activateRelease(releasePath, expectedTreeHash, sha) {
        ...
        const script = `set -eu; base=${shellQuote(base)}; release=${shellQuote(releasePath)}; expected=${shellQuote(expectedTreeHash)}; host=${shellQuote(host)}; old=""; if [ -e "$base/current" ] || [ -L "$base/current" ]; then old=$(readlink -f "$base/current" || true); fi; chown -R root:root "$release"; find "$release" -type d -exec chmod 0755 {} +; find "$release" -type f -exec chmod 0644 {} +; actual=$(cd "$release" && ${treeCommand}); [ "$actual" = "$expected" ]; ln -sfn "$release" "$base/current.next"; mv -Tf "$base/current.next" "$base/current"; served=$(curl -ksS --resolve "$host:443:127.0.0.1" "https://$host/deploy-manifest.json?commit=${sha}" | sed -n 's/.*"treeHash": *"\\([^"]*\\)".*/\\1/p' | head -n 1); if [ "$served" != "$expected" ]; then if [ -n "$old" ]; then ln -sfn "$old" "$base/current.rollback"; mv -Tf "$base/current.rollback" "$base/current"; fi; exit 1; fi; printf 'OLD_TARGET=%s\\n' "$old"; printf 'RELEASE=%s\\n' "$release"`;
