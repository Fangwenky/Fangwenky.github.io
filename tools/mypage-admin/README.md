...
在仓库根目录双击 `start-mypage-admin.command`。启动器会检查本机命令、按需安装依赖、启动只监听 `127.0.0.1` 的后台，并自动打开带一次性口令的浏览器页面。`ssh`、`scp` 和 `tar` 仅在发布时需要，缺少它们不会阻止本地编辑。
...
VPS 部署始终从刚刚推送的 Git 提交创建只读快照，不直接上传仍可能变化的工作区，因此不会夹带本地草稿或未跟踪文件。上传使用跨后台进程的 VPS 部署锁和唯一 release 名，并先打包为单一压缩包；打包时禁用 macOS 扩展属性，VPS 解压后还会清理 AppleDouble `._*` 元数据，防止系统附加文件污染目录哈希。VPS 验证压缩包哈希后才解压，随后等待完整文件树哈希稳定。若文件树仍不一致会重新解压，并在最终失败时报告缺失、多余或内容变化的具体文件。切换前失败会自动删除半成品 release，只有哈希、权限和公网 manifest 全部验证通过后才完成切换。部署进程异常退出后，超过 15 分钟的锁会被下一次任务安全接管。
